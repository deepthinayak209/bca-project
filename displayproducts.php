...
<!-- <input type="text" name="search" class="form-control" placeholder="Enter Name for search" onkeyup="searchit(this.value)"> -->
   ...
   <?php
     $stmt = $admin->ret("SELECT * FROM `product` ORDER BY `product_id` DESC ");
...
<tr>
    ...
    <td><a href="Controller/productdelete.php?product_id=<?php echo $row['product_id']; ?>" OnClick="return confirm('are you sure')">delete</a></td>
    <td><a href="updateproduct.php?product_id=<?php echo $row['product_id']; ?>" OnClick="return confirm('are you sure')">update</a></td>
</tr>
<?php } ?>
</table>
